...
then
    ...
    webpackWatch '../swan-core' & webpackWatch '../swan-components' & webpackWatch $CURRENTPATH &
    print 'swan-app编译完成，已启动webpack监听';
else
    #同时触发本地多模块的编译脚本，为节省时间，此处为并行执行，完成后再触发swan-app的编译脚本;
    build '../swan-core' & build '../swan-components'& build '../swan-api';
    wait;
    print '开始正式编译swan-app';
	webpack $CURRENTPATH;
    mkdir -p output
    print '正在生成pkginfo';
    node tool/make-pkginfo.js "${CURRENTPATH}/dist/box/pkginfo.json"
